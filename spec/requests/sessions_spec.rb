...
RSpec.describe "Sessions", type: :request do
  ...
  User.create(username: 'testuser', email: 'test1@email.com', password: 'password')
  ...
  describe "POST /create" do
    context "with valid login details" do
      it "returns http 302" do
        post "/sessions", params: { 'user[email]' => 'test1@email.com', 'user[password]' => 'password' }
    ...
    context "with invalid login details" do
      it "returns http 302 with invalid password" do
        post "/sessions", params: { 'user[email]' => 'test1@email.com', 'user[password]' => 'pa5sw0rd' }
        expect(response).to have_http_status(302)
      end
