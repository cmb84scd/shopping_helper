  ...
  describe "POST /create" do
    context "with valid login details" do
      it "returns http 302" do
        post "/sessions", params: { 'user[email]' => user1.email, 'user[password]' => user1.password }
    ...
    context "with invalid login details" do
      it "returns http 302 with invalid password" do
        post "/sessions", params: { 'user[email]' => user1.email, 'user[password]' => 'pa5sw0rd' }
        expect(response).to have_http_status(302)
      end

      it "returns http 302 with invalid email" do
        post "/sessions", params: { 'user[email]' => 'user@email.com', 'user[password]' => user1.password }
        expect(response).to have_http_status(302)
      end
